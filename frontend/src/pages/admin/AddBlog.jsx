...
/* Initiate Quill only once */
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 text-gray-600 h-full overflow-scroll bg-messenger-50/50'>
        <div className='bg-white w-full max-w-3xl p-4 md:p-10 sm:m-10 rounded shadow'>
          ...
          <div className='max-w-lg h-74 pb-16  sm:pb-10 pt-2 relative'>
            ...
            {loading && (
              <div className='absolute right-0 top-0 bottom-0 left-0 flex items-center justify-center bg-x/10 mt-2'>
                <div className='w-8 h-8 rounded-full border-2 border-t-white animate-spin'></div>
              </div>)}
            <button disabled={loading} type='button' className='absolute bottom-1 right-2 ml-2 text-xs text-white bg-x/70 px-4 py-1.5 rounded hover:underline cursor-pointer' onClick={generateContent}>Generate with AI</button>
          </div>
          ...
          <button disabled={isAdding} type='submit' className='mt-8 w-40 h-10 bg-telegram text-white rounded cursor-pointer text-sm'>
